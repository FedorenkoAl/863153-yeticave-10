    ...
    <textarea id="message" name="message" placeholder="Напишите описание лота"><?=htmlspecialchars($_POST['message']);?></textarea>
      ...
      <div class="form__container-three">
        ...
        <div class="form__item <?=$errors['lot-date'];?>">
          ...
          <input class="form__input-date" id="lot-date" value="<?=$_POST['lot-date'];?>" type="text" name="lot-date" placeholder="Введите дату в формате ГГГГ-ММ-ДД">
          <span class="form__error"><?=$dict['lot-date'];?></span>
        </div>
      </div>
 <?php if (empty($file_number)) :?>
      <span class="form__error form__error--bottom">Пожалуйста, исправьте ошибки в форме</span>
           <?php elseif (!empty($file_number) && $file_number != 2 && $file_number != 3) : ?>
        <span class="form__error form__error--bottom">Загрузите фотографию</span><?php elseif (!empty($int) && $file_number === 2): ?>
        <span class="form__error form__error--bottom">Загрузите картинку в формате jpg/png</span>
        <?php else : ?>
    <span class="form__error form__error--bottom">Максимальный размер файла: 20Кб</span>
 <?php endif; ?>
